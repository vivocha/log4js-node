...
function getLogger (categoryName) {
  ...
  if (!loggers[categoryName]) {
    ...
    var normalizedLoggerCategory = categoryName + '.';

    var level = undefined;

    if (levels.config) {
      for (var levelCategory in Object.keys(levels.config).sort()) {
        if (levels.config.hasOwnProperty(levelCategory)) {
          var normalizedLevelCategory = levelCategory + '.';
          if (normalizedLoggerCategory.substring(0, normalizedLevelCategory.length) == normalizedLevelCategory) {
    ...
    loggers[categoryName] = new Logger(categoryName, level);

    for(var appenderCategory in appenderList) {
      var normalizedAppenderCategory = appenderCategory + '.';
      if (normalizedLoggerCategory.substring(0, normalizedAppenderCategory.length) == normalizedAppenderCategory) {
  ...
  args.forEach(function(category) {
    ...
    if (category === ALL_CATEGORIES) {
      ...
      for(var loggerCategory in loggers) {
        var normalizedLoggerCategory = loggerCategory + '.';
        if (normalizedLoggerCategory.substring(0, normalizedCategory.length) == normalizedCategory) {
...
function configureLevels(_levels) {
  ...
  if (_levels) {
    for (var category in _levels) {
      if (_levels.hasOwnProperty(category)) {
        var normalizedCategory = category + '.';
        for(var loggerCategory in loggers) {
          var normalizedLoggerCategory = loggerCategory + '.';
          if (normalizedLoggerCategory.substring(0, normalizedCategory.length) == normalizedCategory) {
